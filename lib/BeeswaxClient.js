...
BeeswaxClient.prototype.authenticate = function() {
    ...
    console.log('authenticating'); //TODO
...
BeeswaxClient.prototype.request = function(method, opts) {
    ...
    console.log(JSON.stringify(opts)); //TODO
...
//TODO: decide how this will work here, how much functionality lives here + in cwrx code
BeeswaxClient.prototype._createAssetWithUpload = function(advertiserId, assetName, url) {
    var self = this,
        imgResp, assetId;

    return rp.get({ url: url, resolveWithFullResponse: true })
    .then(function(response) {
        //TODO: handle some errors
        imgResp = response;
        var imgSize = parseInt(imgResp.headers['content-length']); //TODO: probs need to handle NaN
        console.log('got image, size = ', imgSize);
        
        return self.request('post', {
            url: self.apiRoot + '/rest/creative_asset',
            json: {
                advertiser_id: advertiserId,
                creative_asset_name: assetName,
                size_in_bytes: imgSize,
            }
        })
        .finally(function() {
            console.log('post creative asset finally');
        });
    })
    .then(function(resp) {
        // TODO: handle resp.success === false
        assetId = resp.payload.id;
        console.log('posted creative asset, id = ', assetId);
        
        return self.request('post', {
            url: self.apiRoot + '/rest/creative_asset/upload/' + assetId,
            formData: {
                creative_content: imgResp
            }
        });
    })
    .then(function(resp) {
        ...
        return self.request('get', {
            url: self.apiRoot + '/rest/creative_asset/',
            body: { creative_asset_id: assetId }
        });
    })
    .catch(function(error) {
